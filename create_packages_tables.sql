...
CREATE POLICY "Vendors can manage their own packages" ON packages
  FOR ALL USING (
    vendor_id IN (
...
-- Create RLS policies for package_items
CREATE POLICY "Package items are viewable by everyone" ON package_items
  FOR SELECT USING (true);

CREATE POLICY "Vendors can manage items for their packages" ON package_items
  FOR ALL USING (
    package_id IN (
...
-- Create RLS policies for package_addons
CREATE POLICY "Package addons are viewable by everyone" ON package_addons
  FOR SELECT USING (true);

CREATE POLICY "Vendors can manage addons for their packages" ON package_addons
  FOR ALL USING (
    package_id IN (
      SELECT id FROM packages 
      WHERE vendor_id IN (
...
-- Create RLS policies for package_images
CREATE POLICY "Package images are viewable by everyone" ON package_images
  FOR SELECT USING (true);

CREATE POLICY "Vendors can manage images for their packages" ON package_images
  FOR ALL USING (
    package_id IN (
      SELECT id FROM packages 
      WHERE vendor_id IN (
