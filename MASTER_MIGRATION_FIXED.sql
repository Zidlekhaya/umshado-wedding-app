-- ============================================
-- UMSHADO MASTER MIGRATION SCRIPT - FIXED
-- ============================================
-- This script creates tables in the correct dependency order

-- ============================================
-- STEP 1: ENABLE EXTENSIONS & CREATE TYPES
-- ============================================
...
-- 2. Weddings table (depends only on app_user)
...
-- 3. Vendor profiles table (depends only on auth.users)
...
-- 4. Wedding co-owners (depends on wedding and app_user)
...
-- 5. Guests table (depends on wedding)
...
-- 6. Tasks table (depends on wedding and app_user)
...
-- 7. Vendors table (depends on wedding)
...
-- 8. Budget items table (depends on wedding and vendor)
...
-- 9. Timeline events table (depends on wedding and app_user)
...
-- 10. Wedding invites table (depends on wedding and app_user)
...
-- ============================================
-- STEP 4: VENDOR MARKETPLACE TABLES
-- ============================================

-- 11. Vendor packages table (depends on vendor_profiles)
...
-- 12. Vendor inquiries table (depends on vendor_profiles, auth.users, wedding, vendor_packages)
...
-- 14. Messages table (depends on conversations)
...
-- 15. Push notifications tokens table
...
-- ============================================
-- STEP 7: CREATE TRIGGERS & FUNCTIONS
-- ============================================
...
DROP TRIGGER IF EXISTS update_budget_item_updated_at ON budget_item;
...
DROP TRIGGER IF EXISTS update_timeline_event_updated_at ON timeline_event;
...
DROP TRIGGER IF EXISTS update_wedding_invites_updated_at ON wedding_invites;
...
DROP TRIGGER IF EXISTS generate_wedding_slug_trigger ON wedding;
...
-- Wedding policies
DROP POLICY IF EXISTS "Users can view weddings they own or co-own" ON wedding;
...
DROP POLICY IF EXISTS "Owners can update their weddings" ON wedding;
CREATE POLICY "Owners can update their weddings" ON wedding FOR UPDATE USING (owner_id = auth.uid());

DROP POLICY IF EXISTS "Owners can delete their weddings" ON wedding;
...
-- Task policies
DROP POLICY IF EXISTS "Users can manage tasks of their weddings" ON task;
...
-- Budget policies
DROP POLICY IF EXISTS "Users can manage budget items of their weddings" ON budget_item;
...
DROP POLICY IF EXISTS "Anyone can view active vendor profiles" ON vendor_profiles;
CREATE POLICY "Anyone can view active vendor profiles" ON vendor_profiles FOR SELECT USING (is_active = TRUE);

DROP POLICY IF EXISTS "Vendors can manage their own profile" ON vendor_profiles;
CREATE POLICY "Vendors can manage their own profile" ON vendor_profiles FOR ALL USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Anyone can view active packages" ON vendor_packages;
...
DROP POLICY IF EXISTS "Vendors can manage their own packages" ON vendor_packages;
...
DROP POLICY IF EXISTS "Vendors can view inquiries to them" ON vendor_inquiries;
CREATE POLICY "Vendors can view inquiries to them" ON vendor_inquiries FOR SELECT USING (
  vendor_profile_id IN (SELECT id FROM vendor_profiles WHERE user_id = auth.uid())
);

DROP POLICY IF EXISTS "Couples can view their own inquiries" ON vendor_inquiries;
CREATE POLICY "Couples can view their own inquiries" ON vendor_inquiries FOR SELECT USING (couple_id = auth.uid());

DROP POLICY IF EXISTS "Anyone can create inquiries" ON vendor_inquiries;
CREATE POLICY "Anyone can create inquiries" ON vendor_inquiries FOR INSERT WITH CHECK (couple_id = auth.uid());

DROP POLICY IF EXISTS "Vendors can update inquiries to them" ON vendor_inquiries;
